...
// src/jetson-logs/index.ts
var implementation = {
  name: "jetson-logs",
  version: "1.0.0"
};
var CEREBRAS_SYSTEM_PROMPT = `You are an expert NVIDIA Jetson edge AI performance analyst. Given current device metrics and a proposed configuration change, predict the expected performance.
...
function parseCerebrasResponse(aiResponse) {
...
async function loadLogFromBucket(env, filename) {
  try {
...
async function listLogsInBucket(env) {
  try {
...
function computeStats(data, filename) {
  ...
  return {
    ...
    latency: {
      avg_latency_ms: Math.round(avgLatency * 1e3) / 1e3,
      p50_latency_ms: Math.round(percentile(latencies, 0.5) * 1e3) / 1e3,
      p95_latency_ms: Math.round(percentile(latencies, 0.95) * 1e3) / 1e3,
      p99_latency_ms: Math.round(percentile(latencies, 0.99) * 1e3) / 1e3,
      max_latency_ms: Math.round(Math.max(...latencies) * 1e3) / 1e3,
      min_latency_ms: Math.round(Math.min(...latencies) * 1e3) / 1e3,
      jitter_std_ms: Math.round(std(latencies) * 1e3) / 1e3,
      avg_trt_inference_ms: Math.round(avg(trtLatencies) * 1e3) / 1e3
    },
    throughput: {
      estimated_fps: Math.round(1e3 / avgLatency * 100) / 100
    },
    reliability: {
      total_frame_drops: maxDrops,
      drop_rate: Math.round(maxDrops / data.length * 1e4) / 1e4,
      avg_queue_delay_ms: Math.round(avg(queueDelays) * 1e3) / 1e3,
      max_queue_delay_ms: Math.round(Math.max(...queueDelays) * 1e3) / 1e3
    },
    thermal: {
...
var jetson_logs_default = (server, env, state) => {
  server.registerTool(
    "list_logs",
    {
      title: "List Logs",
  ...
  server.registerTool(
    "load_log",
    {
      ...
      inputSchema: {
        filename: external_exports.string().describe("Name of the CSV log file to load (e.g., 'inference_log_20251201_211136.csv')")
      }
    },
    async ({ filename }, { sendNotification }) => {
      await sendNotification({
        method: "notifications/message",
        params: {
          level: "info",
  ...
  server.registerTool(
    "recommend_config",
    {
      ...
      inputSchema: {
        filename: external_exports.string().describe("Name of the log file to analyze for recommendations")
      }
    },
    async ({ filename }, { sendNotification }) => {
      await sendNotification({
  ...
  server.registerTool(
    "predict_latency",
    {
      ...
      inputSchema: {
        current_metrics: external_exports.object({
          avg_latency_ms: external_exports.number().describe("Current average latency in milliseconds"),
          avg_temp_c: external_exports.number().describe("Current average GPU temperature in Celsius"),
          gpu_util_percent: external_exports.number().describe("Current GPU utilization percentage"),
          power_w: external_exports.number().describe("Current power consumption in watts")
        }).describe("Current performance metrics from the device"),
        proposed_config: external_exports.object({
          precision: external_exports.enum(["FP32", "FP16", "INT8"]).describe("Model precision (FP32, FP16, or INT8)"),
          batch_size: external_exports.number().describe("Batch size (1, 2, 4, 8)"),
          resolution: external_exports.number().describe("Input resolution (320, 416, 512, 640)"),
          power_mode: external_exports.enum(["10W", "15W", "30W", "MAXN"]).describe("Jetson power mode")
        }).describe("Proposed deployment configuration to predict")
      }
    },
    async ({ current_metrics, proposed_config }, { sendNotification }) => {
      await sendNotification({
        method: "notifications/message",
        params: {
          level: "info",
...
- GPU temperature: ${current_metrics.avg_temp_c}\xB0C
...
Predict the expected performance for this configuration change.`;
  ...
  server.registerTool(
    "evaluate_config",
    {
      ...
      inputSchema: {
        log_filename: external_exports.string().describe("Name of the log file to use as baseline metrics"),
        proposed_config: external_exports.object({
          precision: external_exports.enum(["FP32", "FP16", "INT8"]).describe("Model precision"),
          batch_size: external_exports.number().describe("Batch size (1, 2, 4, 8)"),
          resolution: external_exports.number().describe("Input resolution (320, 416, 512, 640)"),
          power_mode: external_exports.enum(["10W", "15W", "30W", "MAXN"]).describe("Jetson power mode")
        }).describe("Proposed deployment configuration"),
        constraints: external_exports.object({
          max_latency_ms: external_exports.number().optional().describe("Maximum acceptable latency in ms"),
          max_temp_c: external_exports.number().optional().describe("Maximum acceptable temperature in Celsius"),
          min_fps: external_exports.number().optional().describe("Minimum required FPS"),
          max_power_w: external_exports.number().optional().describe("Maximum power budget in watts")
        }).optional().describe("Optional performance constraints")
      }
    },
    async ({ log_filename, proposed_config, constraints }, { sendNotification }) => {
      await sendNotification({
        method: "notifications/message",
        params: {
          level: "info",
...
Predict the expected performance for this configuration change.`;
  ...
  server.registerTool(
    "upload_log",
    {
      ...
      inputSchema: {
        filename: external_exports.string().describe("Name for the log file (e.g., 'inference_log_20251201_211136.csv')"),
        csv_content: external_exports.string().describe("The full CSV content of the log file, including headers")
      }
    },
    async ({ filename, csv_content }, { sendNotification }) => {
      await sendNotification({
        method: "notifications/message",
        params: {
          level: "info",
  ...
  server.registerTool(
    "delete_log",
    {
      ...
      inputSchema: {
        filename: external_exports.string().describe("Name of the log file to delete")
      }
    },
    async ({ filename }, { sendNotification }) => {
      await sendNotification({
...
// <stdin>
var stdin_default = jetson_logs_default;
export {
  cors,
